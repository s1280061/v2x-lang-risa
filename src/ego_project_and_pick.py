...
def load_json_nearest(dir_path: Path, frame6: str) -> dict:
    ...
    if not cand:
        # non-numeric stems -> just use first
...
def draw_all_bboxes(img: np.ndarray, label_list: list[dict]) -> None:
    """
    Draw every 2D bbox in infra label on the image (white).
    """
...
def bbox_center(b: dict) -> tuple[float, float] | None:
    if not isinstance(b, dict):
        return None
    if not all(k in b for k in ("xmin", "ymin", "xmax", "ymax")):
        return None
...
def process_one_pair(
    ...
    # ===== draw all bboxes =====
    draw_all_bboxes(img, inf_label)

    # ===== ego point + best bbox distance (JSON only) =====
    best = None
    best_d = None
    ...
    if uv is not None:
        ...
        if best_obj is not None:
            best = best_obj
            best_d = float(best_dist)

            # highlight bbox only (NO TEXT)
            b = best.get("2d_box")
            if isinstance(b, dict) and all(k in b for k in ("xmin", "ymin", "xmax", "ymax")):
                xmin, ymin, xmax, ymax = map(int, (b["xmin"], b["ymin"], b["xmax"], b["ymax"]))
                cv2.rectangle(img, (xmin, ymin), (xmax, ymax), (0, 255, 0), 3)  # green
    ...
    dump = {
        ...
        # d is JSON only
        "picked_bbox_center_dist_px": best_d,
        "picked_obj_track_id": best.get("track_id") if best else None,
        "picked_obj_type": best.get("type") if best else None,
